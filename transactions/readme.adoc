...
(`TODO-07`): start a locator and two servers using the cluster/serverStart.gf script script provided in the `cluster` folder:
...
. (`TODO-11`): Use the stop script (`gfsh run --file=serverStop.gf`) to stop the locator and servers. Then, restart and re-run the `TransactionTests` test harness.
