...
. Set the `cache-xml-file` property to point to the server cache xml file.  Note that this will be a relative path to where the servers run.  As a result, you will probably want a path similar to `../cluster.xml`.
